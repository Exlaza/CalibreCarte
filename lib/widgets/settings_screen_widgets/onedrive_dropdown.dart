...
import 'package:calibre_carte/helpers/oauth_constants.dart';
...
class OneDriveDropdown extends StatefulWidget {
//  static const clientID = 'h1csd4yy5cxl0rl';
  static const redirectUri = 'calibrecarte://dropbox';

  @override
  _OneDriveDropdownState createState() => _OneDriveDropdownState();
}

class _OneDriveDropdownState extends State<OneDriveDropdown> {
  Future myFuture;
  ...
  Future<bool> loadingToken() async {
    ...
    if (sp.containsKey(ONE_DRIVE_TOKEN)) {
      dirNames.clear();
      OneDriveEmail = sp.getString('OneDriveEmail');
    ...
    config = await ConfigLoader(secretPath: "one_drive.json").load();
...
//    };
    print(code);
    Map<String, dynamic> json = {"code":code, "scope":"files.read.all", "redirect_uri": OneDriveDropdown.redirectUri, "grant_type":"authorization_code", "code_verifier":OAuthUtils.codeVerifier, "client_id":clientId} ; // make POST request
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done) {
          if (snapshot.data == false) {
            ...
            final url =
                'https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${clientId}&response_type=code&scope=${"files.read.all"}&redirect_uri=${OneDriveDropdown.redirectUri}&code_challenge=$codeChallenge&code_challenge_method=${OAuthUtils.codeChallengeMethod}';
