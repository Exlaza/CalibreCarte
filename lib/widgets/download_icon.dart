...
class _DownloadIconState extends State<DownloadIcon> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(future: checkIfLocalCopyExists(),builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.done) {
      if(snapshot.data==true){
        return Icon(Icons.file_download, color: Colors.blue,);
      }
      else return Icon(Icons.file_download, color: Colors.black12,);
    }
    else return Icon(Icons.file_download, color: Colors.transparent,);
    }
        );
  }
}
